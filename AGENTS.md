...
- 優先度 3: 接続設定を複数記憶してツリーUIから選択したい
- 優先度 5: 結果をJSONでなくテーブル表示したい
- 優先度 4: SQLを簡易的な補完と色付きで入力したい
- 優先度 2: ssh_configのaliasを使えるようにしたい
- 優先度 1: TLS接続時のCA証明書を指定して検証もできるようにする

## Strategy
- 優先度 3: アプリ設定ディレクトリに接続情報JSONを保存。UIはツリー+編集フォーム+削除。検索と並び替えは後回し。
- 優先度 5: `columns`と`rows`からHTMLテーブル生成。まずはシンプルな静的テーブルで、件数増で仮想スクロール検討。
- 優先度 4: 軽量エディタを導入して色付け。補完はキーワードのみ→必要なら接続メタデータから拡張。
- 優先度 2: まずは`ssh -F <config> <alias>`を許可するUI/実装。必要ならconfig解析へ拡張。
- 優先度 1: UIにCAパス入力（ファイルピッカー）。`mysql::SslOpts::with_root_cert_path`で検証対応。
