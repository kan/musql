...
- **接続プロファイルの外部同期（#47）**: 指定パス（Dropbox/OneDrive 等の同期フォルダ）の JSON にプロファイルストアをミラーし複数マシンで共有。同期パスは Rust の AppConfig `sync_path` ファイルに保存（環境依存のため同期対象外）。**書き出しは `save_profiles` 内で自動ミラー**（全ウィンドウの変更を捕捉、同期的なので focus 取り込みの競合なし）。**取り込み**は `sync_import`（起動時/フォーカス時/`profiles:changed` の `refreshProfiles` が呼ぶ）で id ベース merge（既存はファイル優先・ローカル固有は保持）。読み取り/parse 失敗時はローカル一覧にフォールバック（壊れた同期ファイルで一覧が壊れない）、差分がある時だけ保存。パスワードは keyring のみで JSON には出ない。UI は main メニュー「接続の同期…」(`main:sync-settings` → `openSyncModal`)。**割り切り**: 削除は伝播しない（あるマシンで消しても、まだ持つ別マシンの次回書き出しで復活しうる）。競合は「ファイル優先 merge」= 実質セッション単位の last-write-wins。コマンド: `get_sync_path`/`set_sync_path`/`sync_import`/`sync_export`/`pick_sync_path`。
