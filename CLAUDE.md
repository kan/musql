...
- アップデートチェック: Help メニュー「アップデートを確認...」から手動実行。`check_update` コマンド（または `on_menu_event` の `"main:check-update"` ハンドラ）が `tauri-plugin-updater` で更新チェック。更新あれば `update-available` イベントで UI に通知 → info 色バナーを `.app-header` の下に表示。更新なしなら「最新バージョンです。」アラート。「更新して再起動」ボタンで `install_update` コマンド実行 → NSIS passive インストール → アプリ再起動。
...
- アップデートチェック・セルフアップデート: 実装済み。`tauri-plugin-updater` v2 を使用。`tauri.conf.json` に `plugins.updater`（pubkey + endpoint）を設定。Rust 側: `.plugin(tauri_plugin_updater::Builder::new().build())` で登録。Help メニュー「アップデートを確認...」から手動チェック（`on_menu_event` の `"main:check-update"` ハンドラ、または `check_update` コマンド）。更新あれば `update-available` イベントを emit、なければ `menu:action "no-update"` を emit。`install_update` コマンドで `download_and_install()` → `app.restart()`。UI 側: main ウィンドウで `update-available` イベントを listen → info 色の更新バナー（バージョン表示 + 「更新して再起動」ボタン）を表示。更新なしなら「最新バージョンです。」アラート。i18n 対応（`update_available`/`update_install`/`update_installing`/`check_update`/`no_update_available`）。`capabilities/default.json` に `updater:default` パーミッション追加。Ed25519 署名鍵ペアで検証。初回リリース手順: `cargo tauri signer generate` で鍵生成 → 公開鍵を `tauri.conf.json` に設定 → 秘密鍵を GitHub Secrets に設定。
